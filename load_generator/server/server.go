...
func main() {
	...
	// Create a HTTP server to export stats in prom registry to prometheus
	...
	go func(){
		for {
			...
			case  t := <- ticker.C:
				...
				if err := push.New("http://pushgateway:9091", "grpc_server_job").
					Collector(grpcMetrics).
					Grouping("source", "grpc_server").
					Push(); err != nil {
